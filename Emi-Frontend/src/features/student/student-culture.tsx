...
function CultureCard({ item }: { item: StudentCultureItem }) {
  return <Card><CardHeader><div className="flex flex-wrap gap-2"><Badge tone="blue">Published</Badge><Badge tone="neutral">{item.content_type}</Badge></div><h2 className="mt-2 text-xl font-black text-ink">{item.title}</h2></CardHeader><CardContent><p className="text-sm text-slate-600">{item.description ?? "Tanpa deskripsi"}</p><p className="mt-2 text-sm font-bold text-slate-500">Kelas: {item.school_class?.name ?? item.class_id}</p><CultureMediaPreview item={item} /></CardContent></Card>;
}
