...
export function TeacherDictionaryDetail({ entryId }: { entryId: string }) {
  const { token } = useAuth();
  const entryQuery = useQuery({
    queryKey: ["teacher", "dictionary", "entries", entryId],
    queryFn: () => teacherService.dictionaryEntryDetail(token ?? "", entryId),
  ...
  return (
    <div className="grid gap-8">
      <Link className="inline-flex min-h-12 w-fit items-center gap-2 rounded-[var(--radius-control)] border-2 border-border bg-surface px-4 py-2 text-sm font-black text-ink shadow-emi transition-transform hover:-translate-y-0.5 hover:bg-surface-muted" href={teacherRoutes.dictionary}>
            ...
            <CardHeader>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm font-semibold text-muted">Gunakan audio ini sebagai pendamping saat mengajar kosakata Mekongga.</p>
                </div>
              </div>
            </CardHeader>
            <CardContent>
