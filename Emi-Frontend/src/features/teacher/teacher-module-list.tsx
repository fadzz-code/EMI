...
export function TeacherModuleList() {
  ...
  const [archiveTarget, setArchiveTarget] = useState<TeacherClassModule | null>(null);
  const [publishTarget, setPublishTarget] = useState<TeacherClassModule | null>(null);
  ...
  const publishMutation = useMutation({
    mutationFn: (moduleId: string) => teacherService.publishClassModule(token ?? "", moduleId),
    onSuccess: async () => {
  ...
  return (
    <div className="grid gap-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <PageHeader badge="Guru" description="Kelola modul pembelajaran kelas Anda: tambah, ubah, arsipkan, atau hapus sesuai kebutuhan siswa di kelas ini." title="Modul Kelas" />
        <Button disabled={!classId} onClick={() => setCreateOpen(true)} type="button">
                ...
                <Card className="flex h-full flex-col transition hover:-translate-y-1 hover:shadow-emi" key={module.id}>
                  ...
                  <CardContent>
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      <Link className="inline-flex min-h-10 items-center justify-center gap-2 rounded-[var(--radius-control)] border-2 border-border bg-surface px-3 text-xs font-black text-ink transition hover:-translate-y-0.5 hover:bg-surface-muted" href={teacherRoutes.modulePreview(module.id)}>
      ...
      <ConfirmDialog
        confirmLabel={archiveMutation.isPending ? "Mengarsipkan..." : "Arsipkan Modul"}
        confirmVariant="secondary"
        description={archiveTarget ? `Arsipkan modul "${archiveTarget.title}"? Modul tidak akan terlihat oleh siswa.` : ""}
      ...
      <ConfirmDialog
        ...
        description={publishTarget ? `Terbitkan kembali modul "${publishTarget.title}"? Modul akan terlihat oleh siswa lagi.` : ""}
