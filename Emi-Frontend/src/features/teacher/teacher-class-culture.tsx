...
import { contentTypeLabel, statusLabel } from "./teacher-utils";
...
export function TeacherClassCulture({ classId }: { classId: string }) {
  ...
  return (
    <div className="grid gap-6">
      <PageHeader badge="Guru" description="Kelola konten budaya untuk kelas ini, termasuk media, artikel, tautan, dan status terbit." title="Konten Budaya Kelas" />
      ...
      {!query.isLoading && !query.isError ? items.length === 0 ? <Card><CardContent><EmptyState description="Belum ada konten budaya untuk kelas ini." title="Budaya Mekongga kosong" /></CardContent></Card> : (
        <div className="grid gap-4 md:grid-cols-2">
          {items.map((item) => (
            <Card key={item.id}>
              <CardHeader><div className="flex flex-wrap gap-2"><Badge tone={item.status === "published" ? "blue" : item.status === "archived" ? "neutral" : "yellow"}>{statusLabel(item.status)}</Badge><Badge tone="neutral">{contentTypeLabel(item.content_type)}</Badge></div><h2 className="mt-2 text-xl font-black text-ink">{item.title}</h2></CardHeader>
              <CardContent>
...
{item.status !== "published" ? <Button type="button" onClick={() => publishMutation.mutate(item.id)}>Terbitkan</Button> : null}
                   {item.status !== "archived" ? <Button type="button" variant="secondary" onClick={() => { if (confirm(`Arsipkan konten "${item.title}"?`)) archiveMutation.mutate(item.id); }}>Arsipkan</Button> : null}
                   <Button type="button" variant="danger" onClick={() => { if (confirm(`Hapus konten "${item.title}" dari kelas ini? Tindakan ini tidak dapat dibatalkan.`)) deleteMutation.mutate(item.id); }}>Hapus</Button>
                </div>
              </CardContent>
            </Card>
          ))}
