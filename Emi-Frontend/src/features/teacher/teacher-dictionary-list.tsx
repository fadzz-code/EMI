...
import { formatOptional } from "./teacher-utils";

export function TeacherDictionaryList() {
  const { token } = useAuth();
  ...
  const entriesQuery = useQuery({
    queryKey: ["teacher", "dictionary", "entries", filters],
    queryFn: () => teacherService.dictionaryEntries(token ?? "", filters),
    enabled: Boolean(token),
  });
  const categorySourceQuery = useQuery({
    queryKey: ["teacher", "dictionary", "categories-source"],
    queryFn: () => teacherService.dictionaryEntries(token ?? "", { per_page: 100 }),
    enabled: Boolean(token),
  });
  ...
  return (
    <div className="grid gap-8">
      <PageHeader
        badge="Guru"
        description="Cari arti kata, contoh kalimat, dan audio Bahasa Mekongga untuk membantu mengajar."
                ...
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  {entries.map((entry) => (
                    <article className="flex h-full flex-col rounded-2xl border-2 border-border bg-surface p-5 transition hover:-translate-y-1 hover:shadow-emi" key={entry.id}>
                      ...
                      <Link className="mt-4 inline-flex min-h-12 items-center justify-center gap-2 rounded-[var(--radius-control)] border-2 border-border bg-primary px-4 py-2 text-sm font-black text-primary-foreground shadow-emi transition-transform hover:-translate-y-0.5" href={teacherRoutes.dictionaryDetail(entry.id)}>
                        Lihat Detail
