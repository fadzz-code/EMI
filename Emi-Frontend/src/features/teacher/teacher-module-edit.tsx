...
export function TeacherModuleEdit({ moduleId }: { moduleId: string }) {
  ...
  const [deleteLessonTarget, setDeleteLessonTarget] =
    useState<TeacherClassLesson | null>(null);
  const [archiveLessonTarget, setArchiveLessonTarget] =
    useState<TeacherClassLesson | null>(null);
  ...
  const createLessonMutation = useMutation({
    mutationFn: (payload: TeacherLessonPayload) =>
      teacherService.createClassLesson(token ?? "", moduleId, payload),
  ...
  const archiveLessonMutation = useMutation({
    mutationFn: (lessonId: string) =>
      teacherService.archiveClassLesson(token ?? "", lessonId),
    onSuccess: async (lesson) => {
  ...
  const deleteLessonMutation = useMutation({
    mutationFn: (lessonId: string) =>
      teacherService.deleteClassLesson(token ?? "", lessonId),
    onSuccess: async () => {
  ...
  return (
    <div className="grid gap-8">
      <div className="flex flex-wrap gap-3">
        ...
        >
          <ArrowLeft className="size-5" strokeWidth={2.5} /> Kembali ke Daftar
          Modul
        </Link>
        <Link
          className="inline-flex min-h-11 w-fit items-center gap-2 rounded-[var(--radius-control)] border-2 border-border bg-surface px-4 py-2 text-sm font-black text-ink transition hover:-translate-y-0.5 hover:bg-surface-muted"
          href={teacherRoutes.modulePreview(moduleId)}
        >
      ...
      <PageHeader
        badge="Guru"
        description="Ubah judul, deskripsi, atau urutan modul kelas Anda."
        title="Edit Modul"
      />

      {moduleQuery.isLoading ? (
        <LoadingState title="Memuat detail modul" />
      ) : null}
      {moduleQuery.isError ? (
        <ErrorState
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Badge tone={teacherStatusTone(moduleData.status)}>
                  {statusLabel(moduleData.status)}
                </Badge>
              </div>
            </CardHeader>
            <CardContent>
              ...
              >
                <MutationAlert eventKey={Math.max(updateMutation.submittedAt, publishMutation.submittedAt, archiveModuleMutation.submittedAt, createLessonMutation.submittedAt, archiveLessonMutation.submittedAt, deleteLessonMutation.submittedAt)} tone="success" visible={Boolean(successMsg)}>{successMsg}</MutationAlert>
                <MutationAlert eventKey={Math.max(updateMutation.submittedAt, publishMutation.submittedAt, archiveModuleMutation.submittedAt, deleteModuleMutation.submittedAt, createLessonMutation.submittedAt, archiveLessonMutation.submittedAt, deleteLessonMutation.submittedAt)} tone="error" visible={Boolean(actionError)}>{getFirstApiError(actionError)}</MutationAlert>
                ...
                <label className="grid gap-2 text-sm font-black text-primary">
                  ...
                  <Input
                    defaultValue={moduleData.sort_order ?? 1}
                    min={1}
                    name="sort_order"
                    type="number"
                    required
              ...
              <div className="mt-5 flex flex-col gap-3 border-t-2 border-border pt-4 sm:flex-row">
                <Button
                  disabled={
                    archiveModuleMutation.isPending ||
                    moduleData.status === "archived"
                  }
                  onClick={() => {
                    setSuccessMsg(null);
                ...
                <Button
                  disabled={
                    deleteModuleMutation.isPending || moduleAction !== "delete"
                  }
              ...
              {moduleAction !== "delete" ? (
                <p className="mt-2 text-xs font-semibold text-muted">
                  Modul yang masih published harus diarsipkan terlebih dahulu
                  sebelum bisa dihapus permanen.
                </p>
              ) : null}
            </CardContent>
          </Card>
              ...
              {moduleData.lessons && moduleData.lessons.length > 0 ? (
                <div className="grid gap-3">
                  {moduleData.lessons.map((lesson) => (
                    <div
                      className="flex flex-col gap-2 rounded-xl border border-border bg-surface-muted p-3"
                      key={lesson.id}
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <Badge tone={teacherStatusTone(lesson.status)}>
                          {statusLabel(lesson.status)}
                        </Badge>
                      </div>
                      <div className="text-xs text-muted">
                        Tipe: {lesson.content_type || "-"} | Urutan:{" "}
                        {lesson.sort_order}
                      </div>
                      <div className="mt-2 flex flex-wrap gap-2">
                        <Link
                          ...
                          href={teacherRoutes.lessonPreview(
                            moduleId,
                            lesson.id,
                          )}
                        >
                          <Eye className="size-4" strokeWidth={2.5} /> Lihat
                          Materi
                        </Link>
                        <Link
                          className="inline-flex min-h-10 items-center justify-center gap-2 rounded-[var(--radius-control)] border-2 border-border bg-surface px-3 text-xs font-black text-primary transition hover:-translate-y-0.5 hover:bg-primary hover:text-primary-foreground hover:shadow-emi"
                        ...
                        >
                          <Pencil className="size-4" strokeWidth={2.5} /> Edit
                          Materi
                        </Link>
                        ...
                        <button
                          ...
                          title={
                            lessonLifecycle(lesson) !== "delete"
                              ? "Materi yang masih published harus diarsipkan terlebih dahulu sebelum dihapus."
                              : undefined
                          }
                          type="button"
                        >
                ...
                <EmptyState
                  description="Modul ini tidak memiliki materi (lessons)."
                  title="Materi kosong"
                />
              )}
            </CardContent>
      ...
      <ConfirmDialog
        ...
        description={
          moduleData
            ? `Modul "${moduleData.title}" akan dihapus secara permanen dan hanya berlaku untuk kelas ini.`
            : ""
        }
      ...
      <ConfirmDialog
        ...
        description={
          moduleData
            ? `Arsipkan modul "${moduleData.title}"? Modul tidak akan terlihat oleh siswa.`
            : ""
        }
      ...
      <ConfirmDialog
        ...
        description={
          moduleData
            ? `Terbitkan modul "${moduleData.title}"? Modul akan terlihat oleh siswa.`
            : ""
        }
      ...
      <ConfirmDialog
        confirmLabel={
          deleteLessonMutation.isPending ? "Menghapus..." : "Hapus Materi"
        }
        description={
          deleteLessonTarget
            ? `Materi "${deleteLessonTarget.title}" akan dihapus secara permanen dari modul ini.`
            : ""
        }
      ...
      <ConfirmDialog
        confirmLabel={
          archiveLessonMutation.isPending
            ? "Mengarsipkan..."
            : "Arsipkan Materi"
        }
        confirmVariant="secondary"
        description={
          archiveLessonTarget
            ? `Arsipkan materi "${archiveLessonTarget.title}"? Materi tidak akan terlihat oleh siswa.`
            : ""
        }
        isConfirming={archiveLessonMutation.isPending}
        onCancel={() => setArchiveLessonTarget(null)}
        onConfirm={() => {
          if (archiveLessonTarget)
            archiveLessonMutation.mutate(archiveLessonTarget.id);
        }}
