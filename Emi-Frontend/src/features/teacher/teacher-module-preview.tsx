        ...
        <div className="flex-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-xs font-black uppercase text-muted">
              Materi {index + 1} · {contentTypeLabel(lesson.content_type)}
            </span>
          </div>
      ...
      {open ? (
        <div className="mt-4 border-t-2 border-border pt-4">
          {contentQuery.isLoading ? (
            <LoadingState title="Memuat isi materi" />
          ) : null}
          {contentQuery.isError ? (
            <ErrorState
              description={getFirstApiError(contentQuery.error)}
              onRetry={() => void contentQuery.refetch()}
              title="Gagal memuat isi materi"
            />
          ) : null}
          {!contentQuery.isLoading && !contentQuery.isError ? (
            <LessonBody content={contentQuery.data} />
          ) : null}
        </div>
      ) : null}
    </div>
...
export function TeacherModulePreview({ moduleId }: { moduleId: string }) {
  ...
  return (
    <div className="grid gap-8">
      ...
      <PageHeader
        badge="Guru"
        description="Lihat modul persis seperti tampilan yang akan dilihat siswa."
        title="Lihat Modul (Preview)"
      />
      ...
      {moduleData ? (
        <>
          <header className="grid gap-6 rounded-3xl border-2 border-border bg-[var(--color-primary-muted)] p-6 shadow-emi sm:p-8 lg:grid-cols-[1.2fr_auto] lg:items-center">
            <div className="grid gap-5">
              <div className="flex flex-wrap gap-2">
                ...
                <Badge tone="yellow">
                  {formatCount(lessons.length)} materi
                </Badge>
              </div>
              <div>
                ...
                <p className="mt-3 max-w-2xl text-sm font-semibold leading-6 text-muted">
                  {formatOptional(moduleData.description)}
                </p>
              </div>
            </div>
          </header>
          ...
          <Card>
            <CardHeader>
              <h2 className="text-xl font-black text-ink">Detail Modul</h2>
            </CardHeader>
            <CardContent>
              <dl className="grid gap-3 text-sm sm:grid-cols-2">
                <div className="rounded-xl border-2 border-border bg-surface-muted p-3">
                  ...
                  <dd className="mt-1 font-bold text-ink">
                    {formatOptional(moduleData.sort_order)}
                  </dd>
                </div>
                <div className="rounded-xl border-2 border-border bg-surface-muted p-3">
                  <dt className="font-black uppercase text-muted">Terbit</dt>
                  <dd className="mt-1 font-bold text-ink">
                    {formatDate(moduleData.published_at)}
                  </dd>
                </div>
              </dl>
            </CardContent>
          ...
          <Card>
            <CardHeader>
              <h2 className="text-xl font-black text-ink">
                Materi ({formatCount(lessons.length)})
              </h2>
              <p className="mt-1 text-sm font-semibold text-muted">
                Klik salah satu materi untuk membuka dan melihat isinya (teks,
                gambar, audio, PDF, atau video).
              </p>
            </CardHeader>
            <CardContent>
              {lessons.length === 0 ? (
                <EmptyState
                  description="Modul ini belum memiliki materi."
                  title="Materi kosong"
                />
              ) : (
      ...
      {!moduleQuery.isLoading && !moduleQuery.isError && !moduleData ? (
        <Alert tone="warning">Modul tidak ditemukan.</Alert>
      ) : null}
    </div>
  );
}
