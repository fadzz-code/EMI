...
import { Alert, Badge, Button, Card, CardContent, CardHeader, FilePreview, FormField, PageHeader, Select, UploadComponent } from "@/components/ui";
...
export function TeacherMediaPage() {
  ...
  return (
    <div className="grid gap-6">
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-wrap gap-2"><Badge tone="yellow">Galeri ditunda</Badge><Badge tone="blue">Unggah aktif</Badge></div>
          <h2 className="mt-3 text-xl font-black text-ink">Media dipakai dari editor materi dan kuis</h2>
        </CardHeader>
        <CardContent>
          <p className="text-sm leading-6 text-slate-700">
            Saat ini media guru dipakai dari editor materi, builder kuis, dan konten budaya. Halaman ini tetap menyediakan unggahan nyata, tetapi belum menampilkan galeri media umum agar tidak membuat daftar palsu.
          </p>
      ...
      <Card>
        <CardHeader><h2 className="text-xl font-black text-ink">Unggah Media</h2></CardHeader>
        <CardContent>
          <form className="grid gap-4" onSubmit={submit}>
            {uploadMutation.error ? <Alert tone="error">{getFirstApiError(uploadMutation.error)}</Alert> : null}
            {uploaded ? <Alert tone="success">Media berhasil diunggah dan siap dipakai melalui editor materi atau kuis.</Alert> : null}
...
<option value="public">Publik</option>
            ...
            <Button disabled={!file || uploadMutation.isPending} type="submit">{uploadMutation.isPending ? "Mengunggah..." : "Unggah Media"}</Button>
          </form>

          {uploaded ? (
            <div className="mt-6 grid gap-3 rounded-xl border-2 border-ink bg-slate-50 p-4 text-sm">
              <h3 className="font-black text-ink">Detail Media</h3>
...
<div><dt className="font-black text-slate-500">Akses</dt><dd>{uploaded.visibility === "public" ? "Publik" : uploaded.visibility === "private" ? "Privat" : "Belum tersedia"}</dd></div>
                 <div><dt className="font-black text-slate-500">Tautan publik</dt><dd>{uploaded.url ? <a className="font-black text-blue-700 underline" href={uploaded.url} rel="noreferrer" target="_blank">Buka file</a> : "Tidak tersedia untuk media privat"}</dd></div>
              </dl>
            </div>
          ) : null}
        </CardContent>
