...
export function TeacherMediaPage() {
  ...
  return (
    <div className="grid gap-6">
      <PageHeader badge="Partial" description="Media kelas saat ini dipakai dari Lesson Editor dan Quiz Builder. Backend belum menyediakan endpoint galeri/list media." title="Media" />

      <Card>
        <CardHeader>
          <div className="flex flex-wrap gap-2"><Badge tone="yellow">Galeri belum tersedia</Badge><Badge tone="blue">Upload real backend</Badge></div>
          <h2 className="mt-3 text-xl font-black text-ink">Media belum memiliki daftar pustaka</h2>
        </CardHeader>
        <CardContent>
          <p className="text-sm leading-6 text-slate-700">
            Backend menyediakan upload/detail media, tetapi belum menyediakan endpoint list/gallery media untuk guru. File yang diunggah dari halaman ini dikirim ke backend sungguhan, namun halaman ini tidak menyimpan atau menampilkan galeri palsu setelah refresh.
          </p>
          ...
          {uploaded ? (
            <div className="mt-6 grid gap-3 rounded-xl border-2 border-ink bg-slate-50 p-4 text-sm">
              ...
              <dl className="grid gap-3 sm:grid-cols-2">
                ...
                <div><dt className="font-black text-slate-500">Purpose</dt><dd>{uploaded.purpose ?? "Belum tersedia"}</dd></div>
                <div><dt className="font-black text-slate-500">MIME</dt><dd>{uploaded.mime_type ?? "Belum tersedia"}</dd></div>
                <div><dt className="font-black text-slate-500">Ukuran</dt><dd>{formatBytes(uploaded.size_bytes)}</dd></div>
                <div><dt className="font-black text-slate-500">Visibility</dt><dd>{uploaded.visibility ?? "Belum tersedia"}</dd></div>
