...
void main() {
  ...
  Uint8List pttPacket(int value) =>
      encodeHardwarePacket(kHardwareControlPacketType, Uint8List.fromList([value]));

  Uint8List pcmPacket(List<int> pcmBytes) =>
      encodeHardwarePacket(kHardwarePcmPacketType, Uint8List.fromList(pcmBytes));
  ...
  test('a second press while already recording does not reopen the writer', () async {
    await controller.connect();
  ...
  test('discardRecording deletes the captured file and returns to connected', () async {
    await controller.connect();
    connection.emit(pttPacket(kHardwarePttPressed));
    await pumpMicrotasks();
    connection.emit(pcmPacket(List.filled(3200, 1)));
...
Future<void> pumpEventQueueSettled(
  HardwareCaptureController controller,
) async {
