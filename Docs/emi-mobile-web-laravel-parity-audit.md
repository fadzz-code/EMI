...
| Kuis kelas | list/create | `/teacher/quizzes`, `/teacher/classes/{classId}/quizzes` | `/class-quizzes` | GET/POST | active assigned class; foreign class 403 | PARTIAL | backend Phase7 + Flutter `teacher_quizzes_test.dart` | Mobile list/search/status/page dan create draft memakai API nyata; smoke HP `NOT_TESTED`. |
| Quiz builder | detail/edit questions | `/teacher/quizzes/{id}/builder` | `/class-quizzes/{id}`, `/class-quizzes/{id}/questions`, `/quiz-questions/{id}` | GET/POST/PUT/DELETE | quiz class must be active assignment; content locked after publish/attempt | PARTIAL | backend Phase7 + Flutter targeted | Mobile detail/edit quiz dan builder soal tersedia; reorder dan image question belum tersedia. |
| Pertanyaan | CRUD multiple choice/short answer | builder | `/class-quizzes/{id}/questions`, `/quiz-questions/{id}` | POST/GET/PUT/DELETE | Class Quiz policy + active assignment | PARTIAL | backend Phase7 + Flutter targeted | Mobile supports text, points, order, explanation, exact answer/options; fuzzy controls belum tersedia. |
| Pilihan jawaban | add/edit four choices + one correct | builder | same question endpoints | POST/PUT | Class Quiz policy; answer key Guru only | PARTIAL | backend Phase7 + Flutter targeted | Mobile memakai payload options dinamis dengan minimum dua pilihan dan satu jawaban benar. |
| Publish/archive quiz | status actions | quiz list/builder | `/class-quizzes/{id}/publish`, `/archive` | POST | active assignment; valid questions required | PARTIAL | backend Phase7 + Flutter targeted | Mobile publish/archive dengan konfirmasi; smoke HP `NOT_TESTED`. |
| Hasil kuis | attempts/report | `/teacher/quizzes/{id}/results` | `/class-quizzes/{id}/attempts`, `/report` | GET | active assigned class | PARTIAL | backend Phase7 + Flutter targeted | Mobile Guru memiliki daftar attempt; aggregate endpoint `/report` belum dipakai. |
| Detail jawaban siswa | attempt detail | results | `/quiz-attempts/{id}` | GET | Guru hanya attempt dari Class Quiz assignment aktif | PARTIAL | backend Phase7 + Flutter targeted | Mobile Guru menampilkan detail attempt dan selected option tanpa field sensitif siswa. |
...
## Guru Kuis Kelas actual contract update 2026-07-18

Status: `PARTIAL`; `SIAP UNTUK SMOKE TEST GURU KUIS KELAS`, belum `PARITY_COMPLETE` sebelum full verification dan smoke HP.

| Domain | Web feature/route | API | Method | Request | Response | Policy/authorization | Mobile route/status | Test evidence |
|---|---|---|---|---|---|---|---|---|
| Admin Quiz Template | Admin template/builder/apply `/admin/quizzes*` | `/admin/quiz-templates*`, `/admin/quiz-template-questions*`, `/apply` | mixed | template fields, template questions, target classes | template resource; apply membuat snapshot Class Quiz draft | `auth:sanctum` + `role:admin` + template/question policy; Guru tidak memakai endpoint ini | `/admin/quizzes*`; terpisah dari Guru | Backend `Phase7QuizzesAssessmentTest`; Flutter `admin_quiz_templates_test.dart` |
| Teacher Class Quiz | list/create `/teacher/quizzes`; class list `/teacher/classes/{classId}/quizzes` | `/class-quizzes` | GET/POST | list: `search`, `status`, `page`; create: `class_id`, title/description/instructions, duration, max attempts, `show_result`, open/close | paginated Class Quiz atau created draft; class/counts/status | Sanctum + Class Quiz policy; Guru hanya active assigned class, foreign/inactive assignment 403 | `/teacher/quizzes`, `/teacher/quizzes/create`; PARTIAL | Backend Phase7 membuktikan scope/403/401; Flutter targeted membuktikan endpoint/query/payload |
| Teacher Class Quiz detail/status | builder/detail/update/delete/publish/archive | `/class-quizzes/{id}`, `/publish`, `/archive` | GET/PUT/DELETE/POST | update field kuis; status action tanpa payload | Class Quiz dengan class, counts, questions/options dan answer key untuk Guru | Active assignment; content mutation terkunci setelah publish atau attempt | `/teacher/quizzes/:id`, `/:id/edit`; publish/archive tersedia; delete repository belum diekspos UI | Backend Phase7 lock/policy; Flutter targeted repository/model |
| Teacher question builder | create/edit/delete/reorder questions | `/class-quizzes/{id}/questions`, `/quiz-questions/{id}`, `/questions/reorder` | GET/POST/PUT/DELETE/PATCH | `question_type`, `question_text`, points/order/explanation, short answer atau options; reorder `question_ids` | question/options termasuk answer key untuk Guru | Mengikuti parent Class Quiz policy dan lock | `/teacher/quizzes/:quizId/questions/create`, `/:id/edit`; CRUD tersedia, reorder/image/fuzzy controls belum | Backend Phase7; Flutter targeted question endpoint/payload tests |
| Teacher results | results and student answer detail `/teacher/quizzes/{id}/results` | `/class-quizzes/{id}/attempts`, `/report`, `/quiz-attempts/{id}` | GET | quiz/attempt ID | attempts, report aggregate, scoped attempt detail; student resource tanpa email/password | Guru hanya attempt Class Quiz active assigned class | MISSING di mobile Guru | Backend Phase7; Web implementation evidence |
| Student Attempt | list/detail/attempt/result `/student/quizzes*` | `/student/quizzes*`, `/class-quizzes/{id}/attempts`, `/quiz-attempts/{id}`, `/answers/{question_id}`, `/submit` | GET/POST/PUT | own published class quiz; answer option/text; submit wajib `Idempotency-Key` | student quiz tanpa answer key; start/resume attempt; result dapat menyembunyikan skor menurut `show_result` | `role:student`; own active class dan own attempt; cross-student 403 | Student routes existing; bukan scope perubahan Guru | Backend Phase7 cross-user/idempotency/visibility; Flutter `student_quiz_test.dart` |

- Mobile Guru tidak memanggil Admin Quiz Template dan tidak mengubah template global. `class_id` create diambil dari dashboard active class; backend policy tetap sumber kebenaran.
- Web menganggap Class Quiz terkunci saat status bukan `draft` atau sudah memiliki attempt. Backend mengembalikan `QUIZ_CONTENT_LOCKED` untuk mutation setelah attempt.
- Route hasil dan detail attempt Guru tersedia di mobile melalui `/class-quizzes/{id}/attempts` dan `/quiz-attempts/{id}`. Aggregate `/class-quizzes/{id}/report`, reorder, gambar soal, fuzzy controls, delete UI, dan smoke HP tetap `NOT_TESTED`/gap; full Flutter suite serta analyzer lulus pada 2026-07-19.
