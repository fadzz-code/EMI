...
| Admin | `/admin/schools`, `/admin/classes`, `/admin/classes/[classId]` | School/class CRUD, teacher/student assignment | MCP not accessible; legacy docs name `ADMIN-04`, `ADMIN-05` | Low | Separate school/class menus differ from Figma grouping; forms and assignment views remain dense on narrow screens | P1 | Do not alter class assignment API flow. |
...
| Admin | `/admin/knowledge`, `/admin/knowledge/[knowledgeId]` | Basis AI knowledge CRUD/publish/archive/source preview | MCP not accessible; legacy docs name `ADMIN-11`, `ADMIN-12` but stale docs called placeholder | Missing in Figma | Current app has active feature; Figma/legacy docs lag and ingestion forms remain dense | P1 | Preserve manual/link/PDF knowledge management. |
| Admin | `/admin/modules`, `/admin/modules/[moduleId]` | Module templates, lessons, apply-to-class | MCP not accessible; legacy docs name `ADMIN-13`, `ADMIN-14` | Low | Multi-step visibility/apply flow is text-heavy | P1 | Keep publish/apply/publish-class-content guardrail. |
| Admin | `/admin/quizzes`, `/admin/quizzes/create`, `/admin/quizzes/[quizId]`, `/admin/quizzes/[quizId]/questions` | Quiz templates/questions/builder/media | MCP not accessible; legacy docs name `ADMIN-15`, `ADMIN-16` | Low | Builder form/action density remains UI debt | P1 | Preserve image media and answer rules. |
| Admin | `/admin/culture` | Global Budaya Mekongga content for classes | No clear accessible MCP frame | Missing in Figma | Feature was added after/around Figma planning; content forms remain dense | P1 | Must not remove. |
| Admin | `/admin/reports`, `/admin/reports/classes/[classId]`, `/admin/reports/students/[studentId]` | Progress reports/details | MCP not accessible; legacy docs name `ADMIN-17`, `ADMIN-18` | Low | Tables/report cards and export parity still need manual verification | P1 | Do not claim CSV parity until verified in mobile UI. |
| Admin | `/admin/settings` | Application, profile, banner, security, password, and activity settings | MCP not accessible; legacy docs name `ADMIN-19` | Low | Long single-page form remains dense on narrow screens | P1 | Uses active typed settings APIs; broader visual parity remains UI debt. |
