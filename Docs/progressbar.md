...
- [x] Unify admin dashboard stats grid, simplify hero header, and improve operational signals to match Figma desktop.
...
- [x] Admin speaking exercise management UI for creating/editing global target bacaan with audio upload.
...
- [x] Student Speaking UI visual polish completed: focused recorder screen, warm target card, large record control, audio preview/status panel, result hero, and separated AI/teacher feedback without backend/API/Python changes.
- [ ] Student follow-up polish if requested: progress/profile visual hierarchy, quiz attempt custom confirmation/status polish, and browser responsive QA across edited siswa screens.
